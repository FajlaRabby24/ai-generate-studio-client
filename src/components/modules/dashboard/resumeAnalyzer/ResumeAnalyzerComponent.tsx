...
} from "@/services/dashboard/resume-analyzer/resumeAnalyzer.service";
import {
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-border/40 pb-6 shrink-0">
          <div className="space-y-1">
            ...
            <p className="text-muted-foreground text-sm">
              Edit the AI-restructured resume details below and generate a
              styled PDF layout.
            </p>
          </div>
          ...
          <div className="lg:col-span-6 space-y-6 max-h-[78vh] overflow-y-auto pr-2 pb-12">
            ...
            <div className="p-6 rounded-2xl border border-border/60 bg-card/20 backdrop-blur-md space-y-4 shadow-md">
              <h3 className="text-lg font-bold text-foreground flex items-center gap-2 border-b border-border/40 pb-2">
                <FileText className="w-5 h-5 text-primary" /> Personal
                Information
              </h3>
              ...
              {experiences.length === 0 ? (
                ...
                <div className="space-y-4">
                  {experiences.map((job: any, idx: number) => (
                    <div
                      key={idx}
                      className="relative p-4 border border-border/60 rounded-xl bg-background/30 space-y-3 group"
                    >
                      <button
                        onClick={() => {
                          const newExp = experiences.filter(
                ...
                <div className="space-y-4">
                  {educations.map((school: any, idx: number) => (
                    ...
                    >
                      <button
                        onClick={() => {
                          const newEdu = educations.filter(
                            (_: any, i: number) => i !== idx,
                          );
                          setEditableResume({
                            ...editableResume,
                ...
                <div className="space-y-1.5 shrink-0 mt-auto pt-2 border-t border-slate-200">
                  ...
                  <div className="space-y-1.5">
                    {educations.map((school: any, i: number) => (
                      <div
                        key={i}
                        className="flex items-center justify-between text-[8px] text-slate-600"
                      >
                        <span>
                          <strong className="text-slate-800">
                            {school.degree}
                        ...
                        {school.year && (
                          <span className="italic">{school.year}</span>
                        )}
                      </div>
                    ))}
                  </div>
                          ...
                          onClick={() =>
                            setSelectedScan({
                              ...
                              actionableSuggestions:
                                analyzer.actionableSuggestions,
            ...
            <div className="space-y-6 overflow-y-auto pr-1">
              ...
              <div className="flex flex-col md:flex-row items-center gap-6 p-5 rounded-2xl border border-border/40 bg-muted/20">
                <div className="flex flex-col items-center justify-center shrink-0">
                  <div
                    className={`w-24 h-24 rounded-full border-4 flex flex-col items-center justify-center font-extrabold text-2xl ${getScoreColor(selectedScan.atsScore)}`}
                  >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="p-5 rounded-2xl border border-emerald-500/20 bg-emerald-500/5 space-y-3">
                  <h4 className="font-bold text-emerald-500 text-sm flex items-center gap-2">
                    ...
                    {selectedScan.strengths?.length || 0})
                  </h4>
                ...
                <div className="p-5 rounded-2xl border border-rose-500/20 bg-rose-500/5 space-y-3">
                  <h4 className="font-bold text-rose-500 text-sm flex items-center gap-2">
                    ...
                    {selectedScan.weaknesses?.length || 0})
                  </h4>
                  <ul className="space-y-2">
              ...
              <div className="p-5 rounded-2xl border border-border/40 bg-card space-y-3">
                <h4 className="font-bold text-foreground text-sm flex items-center gap-2">
                  <Sparkles className="w-4 h-4 text-primary" /> Missing Industry
                  Keywords ({selectedScan.missingKeywords?.length || 0})
                </h4>
                <div className="flex flex-wrap gap-2">
                  {selectedScan.missingKeywords?.map((item, index) => (
                    <span
                      key={index}
                      className="px-2.5 py-1 rounded-lg bg-muted text-xs font-semibold text-foreground/80 border border-border/40"
                    >
                      {item}
                    </span>
                  ))}
                  {(!selectedScan.missingKeywords ||
                    selectedScan.missingKeywords.length === 0) && (
                    <span className="text-xs text-muted-foreground">
              ...
              <div className="p-5 rounded-2xl border border-border/40 bg-card space-y-3">
                <h4 className="font-bold text-foreground text-sm flex items-center gap-2">
                  <TrendingUp className="w-4 h-4 text-primary" /> Actionable
                  Recommendations
                </h4>
                <ul className="space-y-2.5">
                  ...
                  {(!selectedScan.actionableSuggestions ||
                    selectedScan.actionableSuggestions.length === 0) && (
                    <li className="text-xs text-muted-foreground">
            ...
            <div className="border-t border-border/40 pt-4 flex justify-between gap-3 shrink-0">
              <div className="flex items-center gap-2">
                ...
                {selectedScan.generatedPdfUrl && (
                  <a
                    href={selectedScan.generatedPdfUrl}
                    download
                    target="_blank"
                    rel="noopener noreferrer"
                  >
                    <Button
                      size="sm"
                      variant="outline"
                      className="rounded-xl font-bold flex items-center gap-2 h-10"
                    >
                      <Download className="w-4 h-4" /> Download PDF
                    </Button>
                  </a>
              ...
              <Button
                onClick={() => setSelectedScan(null)}
                variant="secondary"
                className="rounded-xl border border-border h-10 font-bold"
              >
                Close Report
              </Button>
            </div>
